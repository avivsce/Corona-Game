...
def endgame(balls):
    ...
    if count >= len(balls)-1:
        ...
        textFeedback = my_font.render('FeedBack',True,black)
        screen.blit(textFeedback,(600,500))

        if 450<=mouseClick_x<=650 and 400<= mouseClick_y<=450:
            resetgame(balls)

        #if 450<=mouseClick_x<=650 and 450<=mouseClick_y<=600:
            #import feedback.py
